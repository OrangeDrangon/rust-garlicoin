...
//! Garlicoin addresses.
//!
//! Support for ordinary base58 Garlicoin addresses and private keys.
//!
...
//! use garlicoin::secp256k1::rand::thread_rng;
//!
...
//! let address = Address::p2pkh(&public_key, Network::Garlicoin);
//! ```
...
pub enum Error {
    ...
    /// Garlicoin script opcode does not match any known witness version, the script is malformed.
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Error::MalformedWitnessVersion => f.write_str("garlicoin script opcode does not match any known witness version, the script is malformed"),
...
impl WitnessVersion {
    ...
    /// Converts garlicoin script opcode into [`WitnessVersion`] variant.
    ///
    /// # Returns
    ...
    /// Converts garlicoin script [`Instruction`] (parsed opcode) into [`WitnessVersion`] variant.
    ///
    /// # Returns
    ...
    /// version in garlicoin script. Thus, there is no function to directly convert witness version
    /// into a byte since the conversion requires context (garlicoin script or just a version number).
...
impl From<WitnessVersion> for opcodes::All {
    /// Converts [`WitnessVersion`] instance into corresponding Garlicoin scriptopcode (`OP_0`..`OP_16`).
...
serde_string_impl!(Address, "a Garlicoin address");
...
impl Address {
    ...
    /// Checks whether or not the address is following Garlicoin standardness rules.
    ///
    ...
    /// Creates a URI string *garlicoin:address* optimized to be encoded in QR codes.
    ///
    ...
    ///
    /// assert_eq!(address.is_valid_for_network(Network::Garlicoin), false);
    ///
    /// let address: Address = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse().unwrap();
    /// assert!(address.is_valid_for_network(Network::Garlicoin));
...
mod tests {
    ...
    fn test_p2pkh_from_key() {
        ...
        let addr = Address::p2pkh(&key, Garlicoin);
    ...
    #[test]
    fn test_p2wsh() {
        // stolen from Garlicoin transaction 5df912fda4becb1c29e928bec8d64d93e9ba8efa9b5b405bd683c86fd2c65667
        ...
        let addr = Address::p2wsh(&script, Garlicoin);
        assert_eq!(
            &addr.to_string(),
    ...
    #[test]
    fn test_p2shwsh() {
        // stolen from Garlicoin transaction f9ee2be4df05041d0e0a35d7caa3157495ca4f93b233234c9967b6901dacf7a9
        ...
        let addr = Address::p2shwsh(&script, Garlicoin);
