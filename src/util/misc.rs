...
/// The prefix for signed messages using Garlicoin's message signing protocol.
...
mod message_signing {
    ...
    /// An error used for dealing with Garlicoin Signed Messages.
    #[cfg_attr(docsrs, doc(cfg(feature = "secp-recovery")))]
    ...
    /// A signature on a Garlicoin Signed Message.
    ///
...
/// Hash message for signature using Garlicoin's message signing format.
...
mod tests {
    ...
    fn test_message_signature() {
        ...
        let message = "rust-garlicoin MessageSignature test";
        ...
        assert_eq!(
            pubkey.inner,
            secp256k1::PublicKey::from_secret_key(&secp, &privkey)
        );

        let p2pkh = ::Address::p2pkh(&pubkey, ::Network::Garlicoin);
        assert_eq!(
            signature2.is_signed_by_address(&secp, &p2pkh, msg_hash),
            Ok(true)
        );
        let p2wpkh = ::Address::p2wpkh(&pubkey, ::Network::Garlicoin).unwrap();
        assert_eq!(
            signature2.is_signed_by_address(&secp, &p2wpkh, msg_hash),
            Ok(false)
        );
        let p2shwpkh = ::Address::p2shwpkh(&pubkey, ::Network::Garlicoin).unwrap();
        assert_eq!(
