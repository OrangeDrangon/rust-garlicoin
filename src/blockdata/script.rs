...
//! Garlicoin scripts.
//!
//! Scripts define Garlicoin's digital signature scheme: a signature is formed
...
//! the script leaves `TRUE` on the stack after being evaluated. Garlicoin's
...
pub enum Error {
    ...
    /// `https://github.com/garlicoin/bips/blob/master/bip-0062.mediawiki#Push_operators`
    NonMinimalPush,
...
impl Script {
    ...
    pub fn is_provably_unspendable(&self) -> bool {
        !self.0.is_empty()
            && (opcodes::All::from(self.0[0]).classify(opcodes::ClassifyContext::Legacy)
                == opcodes::Class::ReturnOp
                || opcodes::All::from(self.0[0]).classify(opcodes::ClassifyContext::Legacy)
                    == opcodes::Class::IllegalOp)
    }

    /// Gets the minimum value an output with this script should have in order to be
    /// broadcastable on today's garlicoin network.
    pub fn dust_value(&self) -> ::Amount {
        // This must never be lower than Garlicoin Core's GetDustThreshold() (as of v0.21) as it may
...
mod test {
    ...
    fn script_asm() {
        ...
        assert_eq!(
            hex_script!("4dffff01").asm(),
            "OP_PUSHDATA2 <push past end>"
        );
        assert_eq!(
            hex_script!("4effffffff01").asm(),
    ...
    fn test_bitcoinconsensus() {
        ...
        spent
            .verify(0, ::Amount::from_sat(18393430), spending.as_slice())
            .unwrap();
    }
