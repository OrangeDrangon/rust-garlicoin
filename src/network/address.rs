...
//! Garlicoin network addresses.
//!
//! This module defines the structures and functions needed to encode
//! network addresses in Garlicoin messages.
//!
...
use consensus::encode::{self, Decodable, Encodable, ReadExt, VarInt, WriteExt};
use io;
use network::constants::ServiceFlags;

/// A message which can be sent on the Garlicoin network
...
mod test {
    ...
    fn serialize_addrv2_test() {
        // Taken from https://github.com/garlicoin/garlicoin/blob/12a1c3ad1a43634d2a98717e49e3f02c4acea2fe/src/test/net_tests.cpp#L348
        ...
        assert_eq!(
            serialize(&ip),
            Vec::from_hex("02101a1b2a2b3a3b4a4b5a5b6a6b7a7b8a8b").unwrap()
        );

        let ip = AddrV2::TorV2(FromHex::from_hex("f1f2f3f4f5f6f7f8f9fa").unwrap());
        assert_eq!(
            serialize(&ip),
            Vec::from_hex("030af1f2f3f4f5f6f7f8f9fa").unwrap()
        );
        ...
        assert_eq!(
            serialize(&ip),
            Vec::from_hex("0610fc000001000200030004000500060007").unwrap()
        );
    ...
    fn deserialize_addrv2_test() {
        // Taken from https://github.com/garlicoin/garlicoin/blob/12a1c3ad1a43634d2a98717e49e3f02c4acea2fe/src/test/net_tests.cpp#L386
