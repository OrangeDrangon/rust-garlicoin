...
fn do_test(data: &[u8]) {
    let tx_result: Result<garlicoin::blockdata::transaction::Transaction, _> = garlicoin::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {},
        Ok(mut tx) => {
            let ser = garlicoin::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            let calculated_weight = tx.get_weight();
            for input in &mut tx.input {
                input.witness = garlicoin::blockdata::witness::Witness::default();
            }
            let no_witness_len = garlicoin::consensus::encode::serialize(&tx).len();
